...
**PR/MR body.** Always compose the REQUIRED template below — it is the only
shape and must never be replaced wholesale. `AI_FACTORY_PR_BODY`, when set,
may carry context from the controller; fold any useful content into the
`Changes` field, but never ship it verbatim — it is often an auto-generated
placeholder such as "Generated by ai-factory for <url>". Every field is
REQUIRED; write `None` when a field has no content:
